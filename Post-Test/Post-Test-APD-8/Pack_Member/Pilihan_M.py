...
def pil2_member(laporan):
    ...
    try:
        ...
        laporan[0]["laporan"].append(masalah)
...
def pil3_member(inventaris_U):
    ...
    print("="*58)
    if len(inventaris_U[0]["hadiah"]) == 0:
        print("Belum Ada Hadiah dari Admin")
        input("Tekan Enter Untuk Kembali ke Menu Utama")
        os.system('cls' if os.name == 'nt' else 'clear')
    else:
        tabel = PrettyTable()
        tabel.field_names = ["No"] + list(inventaris_U[0].keys())
        for i,o in enumerate(inventaris_U[0]["hadiah"], start = 1):
            tabel.add_row([i] + [o])
        print(tabel) 
        input("Tekan Enter Untuk Kembali ke Menu Utama")
        os.system('cls' if os.name == 'nt' else 'clear') 
