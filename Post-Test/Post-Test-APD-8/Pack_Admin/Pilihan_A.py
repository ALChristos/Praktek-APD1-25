...
def pil2_admin(tanaman):
    ...
    while ulang_pilihan:
        ...
        lagi = input("Apakah Ingin Menambah Tanaman Lagi?(Y/N)")  
        if lagi.lower() == "y":
            os.system("cls" if os.name == "nt" else "clear")
...
def pil5_admin(tanaman,stok_PV):
     ulang_pilihan = True
     while ulang_pilihan:
        jawab = menu_pil5(stok_PV)
       
        if jawab["menu5"][0] == "1":
            pil5_1(stok_PV)
            
        elif jawab["menu5"][0] == "2":
            pil5_2(stok_PV)
                  
        elif jawab["menu5"][0] == "3":
            pil5_3(stok_PV, tanaman) 
               
        elif jawab["menu5"][0] == "4":
            pil5_4(tanaman)
        
        elif jawab["menu5"][0] == "5":
            os.system("cls" if os.name == "nt" else "clear")   
            ulang_pilihan = False        
...
def pil6_admin(laporan, inventaris_U):
    ...
    if len(laporan[0]["laporan"]) == 0:
        ...
        if penyelesaian <= len(laporan[0]["laporan"]):
            index = penyelesaian - 1
            print(f"Masalah Dengan Seri {laporan[0]["laporan"].pop(index)}\n DI ATASI!")
            hadiah = input("Berikan Hadiah Kepada Member yang Melapor: ")
            inventaris_U[0]["hadiah"].append(hadiah)
